  ...
  const validateForm = () => {
    try {
      if (mode === 'login') {
        ...
        forgotSchema.parse({ email: formData.email });
      }
      setErrors({});
      return true;
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'login') {
        ...
        const { createRecovery } = await import('../services/appwrite');
        await createRecovery(formData.email);
        setIsSuccess(true);
      }
      if (onAuth && mode !== 'forgot') onAuth(mode === 'register', formData.email);
    } catch (err: any) {
      console.error(err);
        ...
        <div className="p-8">
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-black text-text-primary mb-2">
              {mode === 'login' ? 'Welcome Back' : mode === 'register' ? 'Join TraderLense' : 'Reset Password'}
            </h2>
          ...
          {isSuccess && mode === 'forgot' ? (
            <div className="text-center animate-in zoom-in">
              <div className="mb-6 p-4 bg-status-high/10 border border-status-high/20 rounded-xl text-status-high text-sm">
                Check your email! We've sent a password reset link to <span className="font-bold">{formData.email}</span>.
              </div>
              <button
                onClick={() => setMode('login')}
                className="font-bold text-status-neutral hover:underline"
              >
                Back to login
              </button>
            </div>
            ...
            <>
              ...
              >
                ...
                {mode === 'login' ? 'Sign in with Google' : mode === 'register' ? 'Join with Google' : 'Continue with Google'}
              </button>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                >
                  {isLoading ? 'Processing...' : mode === 'login' ? 'Log In' : mode === 'register' ? 'Create Account' : 'Send Reset Link'}
