...
export const JoinCommunityModal: React.FC<JoinCommunityModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Use real Appwrite function
      const { registerForHive } = await import('../services/appwrite');
      await registerForHive(email, preference);
      setIsSuccess(true);
      setTimeout(() => {
        onSubmit(email, preference);
        onClose();
        setIsSuccess(false); // Reset for next time if component stays mounted
      }, 2000);
    } catch (error) {
      ...
      <div className="bg-background-secondary border border-surface rounded-2xl w-full max-w-md shadow-2xl flex flex-col relative overflow-hidden">
        ...
        {isSuccess ? (
          <div className="flex flex-col items-center justify-center p-12 text-center animate-in fade-in zoom-in">
            <div className="w-16 h-16 bg-status-high text-background-primary rounded-full flex items-center justify-center mb-4">
              <CheckCircle2 size={32} />
            </div>
            <h3 className="text-2xl font-bold text-text-primary">Welcome to the Hive!</h3>
            <p className="text-text-muted mt-2">You've secured your spot. Watch your inbox.</p>
          </div>
        ) : (
              ...
              <div className="grid grid-cols-2 gap-3">
                {markets.map((m) => (
                  <button
                    ...
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all text-sm font-bold ${preference === m.id
